<template>
    <div class="view-header">
        <div class="filter-tabs">
            <button class="filter-tab" :class="{ active: redemptionFilter === 'all' }" @click="setFilter('all')">
                Todos
            </button>
    ...
    <div class="table-container">
        ...
        <div class="pagination-controls" v-if="paginationInfo.last_page > 1">
            <button :disabled="paginationInfo.current_page === 1"
                @click="fetchRedemptions(paginationInfo.current_page - 1)">
                Anterior
            </button>
...
<script setup>
import { ref, computed, onMounted } from 'vue';// Asegura la importación correcta
...
const paginationInfo = ref({ current_page: 1, last_page: 1 });
// Función para cargar datos desde el Backend
const fetchRedemptions = async (page = 1) => {
    isLoading.value = true;
    try {
        // Enviamos el filtro actual al backend
        const response = await redemptionService.getAllRedemptions(page, redemptionFilter.value);
...
const filteredRedemptions = computed(() => {
    if (redemptionFilter.value === 'all') return redemptions.value;
    return redemptions.value.filter(r => r.status === redemptionFilter.value);
});
